...
class Zubat(pygame.sprite.Sprite):
    def __init__(self, filename) -> None:
        ...
        self.speed = 6
        self.fly = Animation([f"zubat{i}.png" for i in range(3, 5)], False, 200)  #animations Objekt 3, 5
        self.up_down = Animation([f"zubat{i}.png" for i in range(0, 8)], False, 50)

    def animate(self, anim):
        self.image = anim.next()                       #animation
    ...
    def update(self):
        ...
        if press[pygame.K_UP]:
            ...
            self.animate(self.up_down)                                      #update aufrufen
...
class Game(object):
    ...
    def symbol_count(self, count, group, image, offset):
        if count  != len(group):
            group.empty()
